...
namespace Foam
{
    defineTypeNameAndDebug(ILU0Paddy, 0);

    lduPreconditioner::
        addasymMatrixConstructorToTable<ILU0Paddy>
        addILU0PaddyditionerAsymMatrixConstructorToTable_;
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::ILU0Paddy::calcPreconDiag()
{
...
void Foam::ILU0Paddy::precondition
(
    scalarField& x,
    const scalarField& b,
...
{
   Info << "Hello from Paddy" <<endl;
    if (matrix_.symmetric())
    {
        FatalErrorIn
        (
            "void ILU0Paddy::precondition\n"
            "(\n"
            "    scalarField& x,\n"
            "    const scalarField& b,\n"
            "    const direction cmpt\n"
            ") const"
        )   << "Calling ILU0Paddy on a symetric matrix.  "
...
void Foam::ILU0Paddy::preconditionT
(
    scalarField& x,
    const scalarField& b,
    const direction cmpt
