...
namespace Foam
{
    defineTypeNameAndDebug(ILUC0Before, 0);

    // Register with symmetric and asymmetric run-time selection table
    // HJ and VV, 31/Oct/2017
    lduPreconditioner::
        addsymMatrixConstructorToTable<ILUC0Before>
        addILUC0BeforeditionerSymMatrixConstructorToTable_;

    lduPreconditioner::
        addasymMatrixConstructorToTable<ILUC0Before>
        addILUC0BeforeditionerAsymMatrixConstructorToTable_;
}
...
void Foam::ILUC0Before::precondition
(
    scalarField& x,
    const scalarField& b,
...
) const
{
    ...
    else
    {
        WarningIn
        (
            "void ILUC0Before::precondition"
            "(scalarField& x, const scalarField& b, const direction cmpt)"
        )   << "Unnecessary use of ILUC0Before preconditioner for diagonal matrix. "
            << nl
...
void Foam::ILUC0Before::preconditionT
(
    scalarField& x,
    const scalarField& b,
    const direction cmpt
) const
{
    ...
    else
    {
        WarningIn
        (
            "void ILUC0Before::preconditionT"
            "(scalarField& x, const scalarField& b, const direction cmpt)"
        )   << "Unnecessary use of ILUC0Before preconditioner for diagonal matrix. "
            << nl
            << "Use diagonal preconditioner instead."
