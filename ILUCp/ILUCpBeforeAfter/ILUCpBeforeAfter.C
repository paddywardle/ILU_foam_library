...
namespace Foam
{
    defineTypeNameAndDebug(ILUCpBeforeAfter, 0);

    lduPreconditioner::
        addasymMatrixConstructorToTable<ILUCpBeforeAfter>
        addILUCpBeforeAfterPreconditionerAsymMatrixConstructorToTable_;
    ...
    lduPreconditioner::
        addsymMatrixConstructorToTable<ILUCpBeforeAfter>
        addILUCpBeforeAfterPreconditionerSymMatrixConstructorToTable_;
}
...
void Foam::ILUCpBeforeAfter::calcFactorization()
{
    if (!matrix_.diagonal())
    {
...
Foam::ILUCpBeforeAfter::ILUCpBeforeAfter
(
...
void Foam::ILUCpBeforeAfter::precondition
(
    scalarField& x,
    const scalarField& b,
...
) const
{
    ...
    else
    {
        WarningIn
        (
            "void ILUCpBeforeAfter::precondition"
            "(scalarField& x, const scalarField& b, const direction cmpt)"
        )   << "Unnecessary use of ILUCpBeforeAfter preconditioner for diagonal matrix. "
            << nl
...
void Foam::ILUCpBeforeAfter::preconditionT
(
    scalarField& x,
    const scalarField& b,
    const direction cmpt
) const
{
    ...
    else
    {
        WarningIn
        (
            "void ILUCpBeforeAfter::preconditionT"
            "(scalarField& x, const scalarField& b, const direction cmpt)"
        )   << "Unnecessary use of ILUCpBeforeAfter preconditioner for diagonal matrix. "
            << nl
            << "Use diagonal preconditioner instead."
