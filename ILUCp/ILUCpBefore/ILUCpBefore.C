...
namespace Foam
{
    defineTypeNameAndDebug(ILUCpBefore, 0);

    lduPreconditioner::
        addasymMatrixConstructorToTable<ILUCpBefore>
        addILUCpBeforePreconditionerAsymMatrixConstructorToTable_;
    ...
    lduPreconditioner::
        addsymMatrixConstructorToTable<ILUCpBefore>
        addILUCpBeforePreconditionerSymMatrixConstructorToTable_;
}
...
void Foam::ILUCpBefore::calcFactorization()
{
    if (!matrix_.diagonal())
    {
...
void Foam::ILUCpBefore::precondition
(
    scalarField& x,
    const scalarField& b,
...
) const
{
    ...
    else
    {
        WarningIn
        (
            "void ILUCpBefore::precondition"
            "(scalarField& x, const scalarField& b, const direction cmpt)"
        )   << "Unnecessary use of ILUCpBefore preconditioner for diagonal matrix. "
            << nl
...
void Foam::ILUCpBefore::preconditionT
(
    scalarField& x,
    const scalarField& b,
    const direction cmpt
) const
{
    ...
    else
    {
        WarningIn
        (
            "void ILUCpBefore::preconditionT"
            "(scalarField& x, const scalarField& b, const direction cmpt)"
        )   << "Unnecessary use of ILUCpBefore preconditioner for diagonal matrix. "
            << nl
            << "Use diagonal preconditioner instead."
